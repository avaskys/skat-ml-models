#!/usr/bin/env python3
"""
Train BiddingEvaluator (dense) or BiddingTransformer (attention-based) models.

These models predict win probability at each bid level from the initial 10-card hand.

Usage:
    # Train transformer model (recommended)
    python scripts/train_bidding.py --sgf data/games.sgf --model transformer

    # Train dense model
    python scripts/train_bidding.py --sgf data/games.sgf --model dense

    # With torch.compile (CUDA only)
    python scripts/train_bidding.py --sgf data/games.sgf --model transformer --compile
"""
...
from skat_ml.constants import BIDDING_EVALUATOR_INPUT_DIM
from skat_ml.datasets import StreamingBiddingDataset, bidding_collate_fn
from skat_ml.features import extract_bidding_features
from skat_ml.models import BiddingEvaluator, BiddingTransformer, BinaryFocalLoss, masked_bce_loss
...
def train_model(args, defaults: dict = None):
    ...
    run_dir, run_name = setup_run_directory(output_dir, args.run_name, f"bidding_{args.model}")
    latest_path = run_dir / "bidding_latest.pt"
    ...
    est_samples = int(total_games * 0.9)  # Train split is 90%
    ...
    train_dataset = StreamingBiddingDataset(Path(args.sgf), split="train")
    val_dataset = StreamingBiddingDataset(Path(args.sgf), split="val", shuffle_buffer=0)  # No shuffle needed for val
    ...
    if use_focal:
        focal_loss_fn = BinaryFocalLoss(gamma=args.gamma, reduction="none")
        print(f"Using focal loss (gamma={args.gamma}) with {'sample weights' if use_weights else 'no weights'}")
    else:
        print(f"Using masked BCE loss with {'sample weights' if use_weights else 'no weights'}")
    ...
    config = {
        ...
        "sgf": args.sgf,
        "focal": use_focal,
        "gamma": args.gamma if use_focal else None,
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            label_pickup = batch["label_pickup"].to(device)
            label_hand = batch["label_hand"].to(device)
            ...
            with torch.autocast(device_type=device.type, enabled=use_amp):
                if args.model == "transformer":
                    pred_pickup, pred_hand = model(hand_cards, position)
                else:
                    # Dense model needs feature extraction
                    batch_size = hand_cards.size(0)
                    features = torch.zeros(batch_size, BIDDING_EVALUATOR_INPUT_DIM, device=device)
                    for i in range(batch_size):
                        cards = hand_cards[i].cpu().numpy().tolist()
                        pos = position[i].item()
                        feat = extract_bidding_features(cards, pos)
                        features[i] = torch.from_numpy(feat)
                    pred_pickup, pred_hand = model(features)

                if use_focal:
                    mask_pickup = label_pickup >= 0
                    mask_hand = label_hand >= 0

                    if mask_pickup.any():
                        fl_pickup = focal_loss_fn(pred_pickup[mask_pickup], label_pickup[mask_pickup])
                        if use_weights:
                            w_pickup = weight.unsqueeze(1).expand_as(pred_pickup)[mask_pickup]
                            loss_pickup = (fl_pickup * w_pickup).mean()
                        else:
                            loss_pickup = fl_pickup.mean()
                    else:
                        loss_pickup = torch.tensor(0.0, device=device)

                    if mask_hand.any():
                        fl_hand = focal_loss_fn(pred_hand[mask_hand], label_hand[mask_hand])
                        if use_weights:
                            w_hand = weight.unsqueeze(1).expand_as(pred_hand)[mask_hand]
                            loss_hand = (fl_hand * w_hand).mean()
                        else:
                            loss_hand = fl_hand.mean()
                    else:
                        loss_hand = torch.tensor(0.0, device=device)
                else:
                    w = weight if use_weights else None
                    loss_pickup = masked_bce_loss(pred_pickup, label_pickup, w)
                    loss_hand = masked_bce_loss(pred_hand, label_hand, w)

                loss = loss_pickup + loss_hand
            ...
            train_samples += hand_cards.size(0)
            global_step += 1
            ...
            if global_step % 500 == 0:
                ...
                writer.add_scalar("Train/Loss_Pickup", loss_pickup.item(), global_step)
                writer.add_scalar("Train/Loss_Hand", loss_hand.item(), global_step)
        ...
        if avg_val_loss < best_val_loss:
            ...
            save_best_model(run_dir / "bidding_best.pt", model, config)

        save_checkpoint(
            run_dir / "bidding_latest.pt",
...
def main():
    parser = argparse.ArgumentParser(description="Train Bidding models")
    ...
    parser.add_argument("--no-weights", action="store_true", help="Disable sample weighting")
